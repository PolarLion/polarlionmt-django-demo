...
class ServerModel:
    ...
    def load(self):
        ...
        if self.tokenizer_opt is not None:
            ...
            if self.tokenizer_opt['type'] == 'sentencepiece':
                ...
                dec_model_path = os.path.join(self.model_root, self.tokenizer_opt['dec-model'])

                sp.Load(model_path)
                dec_sp.Load(dec_model_path)
                self.tokenizer = sp
                self.dec_tokenizer = dec_sp
    ...
    def run(self, inputs):
        ...
        if sscount > 0:
            try:
                scores, predictions, attention = self.translator.translate(
        ...
        return src_texts, tgt_texts, results, avg_scores, self.opt.n_best, attention, timer.times
    ...
    def detokenize(self, sequence):
        ...
        if self.tokenizer_opt["type"] == "sentencepiece":
            detok = self.dec_tokenizer.DecodePieces(sequence.split())
        elif self.tokenizer_opt["type"] == "bpe_onmt_tokenizer":
